...
from ylib.scipy_misc import imread, imresize
...
class SegmentationData(AbstractSegmentation):
    ...
    def __init__(self, directory, categories=None, require_all=False):
        ...
        for cat in self.category:
            ...
            self.category_label[cat] = build_dense_label_array(
                    c_data, key='code')
        self.label_pc = self.primary_categories_per_index()
        self.labelcat = self.onehot(self.label_pc)
        self.labelcat_namemap = self.build_namemap()


    def build_namemap(self):
        map = {}
        for li, ci in enumerate(self.label_pc):
            map[self.label[li]['name']] = self.category_names()[ci]
        return map
    ...
    def primary_categories_per_index(ds):
        ...
        for i in range(ds.label_size(None)):
            maxcov, maxcat = max(
                (ds.coverage(cat, catmap[cat][i]) if catmap[cat][i] else -1, ic)
...
class BrodenDataset(dataset.Dataset):

    def __init__(self, root, transform=None, target_transform=None, segmentation_shape=None, categories=("object", "part", "scene", "material", "texture", "color")):
        # super(BrodenDataset, self).__init__(root, transform=transform, target_transform=target_transform)
        self.root = root
        self.categories = categories
        self.data = SegmentationData(root, categories=categories)
        self.segmentation_shape = segmentation_shape
        self.transform = transform

    def __getitem__(self, index):
        filename = self.data.filename(index)
        segs, shape = self.data.__class__.resolve_segmentation(self.data.metadata(index), categories=self.categories)
        if self.segmentation_shape is not None:
            for k, v in segs.items():
                segs[k] = scale_segmentation(v, self.segmentation_shape)
            shape = self.segmentation_shape
        segs['sh'], segs['sw'] = shape
        segs['i'] = index
        segs['fn'] = filename
        segs['image'] = PIL.Image.open(filename).convert('RGB')
        if self.transform is not None:
            segs['tensor'] = self.transform(segs['image'])
        else:
            segs['tensor'] = torch.from_numpy(segs['image'])
        return segs
...
if __name__ == '__main__':
    # data = SegmentationData('../datasets/broden/broden1_224', categories=["object", "part", "scene", "material", "texture", "color"])
    # pd = SegmentationPrefetcher(data, categories=data.category_names()+['image'], once=True)
    # bs = pd.batches().next()


    dataset = BrodenDataset('../datasets/broden/broden1_224', categories=["object", "part", "scene", "material", "texture", "color"])
    dataloader = dataloader.DataLoader(dataset, batch_size=4, shuffle=True, num_workers=2, collate_fn=broden_collate)
    _, batch = enumerate(dataloader).__next__()
    # plt.imshow((imresize(d['color'][0], (224, 224), interp='nearest')[:, :, None] * 0.5 + d['image']).astype(np.uint8));
    # plt.show()
